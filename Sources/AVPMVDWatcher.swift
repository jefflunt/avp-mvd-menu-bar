...
public class AVPMVDWatcher: ObservableObject {
    ...
    public func disconnectMVD() {
        if !scriptExecutor.isProcessTrusted {
            let alert = NSAlert()
            alert.messageText = "Accessibility Permission Required"
            alert.informativeText = "To disconnect from your Apple Vision Pro directly, please grant Accessibility permissions to AVPMVDMenuBar in System Settings.\n\nAfter opening Settings, click the '+' button and add /Applications/AVPMVDMenuBar.app."
            alert.addButton(withTitle: "Open Settings")
            alert.addButton(withTitle: "Cancel")
            alert.alertStyle = .warning
        ...
        tell application "System Events"
            tell process "ControlCenter"
                ...
                -- 3. Click the target device in the list (preferring Sidecar/MVD that is active/checked)
                tell window 1
                    tell group 1
                        tell scroll area 1
                            tell group 1
                                ...
                                -- Pass 1: Try to find a Sidecar checkbox matching deviceName that is checked/value is 1
                                repeat with cb in allCheckboxes
                                    try
                                        set cbId to value of attribute "AXIdentifier" of cb
                                        ...
                                        set cbTitle to title of cb
                                        set cbName to name of cb
                                        if cbId starts with "screen-mirroring-device-Sidecar" and cbValue is 1 then
                                            if cbId contains targetName or cbTitle contains targetName or cbName contains targetName then
                                                set chosenBtn to cb
                                                exit repeat
                                            end if
                                ...
                                -- Pass 2: Try to find any Sidecar checkbox containing AVP keywords that is checked/value is 1
                                if chosenBtn is missing value then
                                    set keywords to {"avp", "vision", "reality", "headset"}
                                    repeat with kw in keywords
                                        if chosenBtn is missing value then
                                ...
                                -- Pass 3: Try to find any Sidecar checkbox that is checked/value is 1
                                if chosenBtn is missing value then
                                    repeat with cb in allCheckboxes
                                        try
                                            set cbId to value of attribute "AXIdentifier" of cb
                                ...
                                -- Pass 4: Try to find an AirPlay checkbox matching deviceName that is checked/value is 1
                                if chosenBtn is missing value then
                                    repeat with cb in allCheckboxes
                                        try
                                            set cbId to value of attribute "AXIdentifier" of cb
                                            ...
                                            set cbTitle to title of cb
                                            set cbName to name of cb
                                            if cbId starts with "screen-mirroring-device-AirPlay" and cbValue is 1 then
                                                if cbId contains targetName or cbTitle contains targetName or cbName contains targetName then
                                                    set chosenBtn to cb
                                                    exit repeat
                                                end if
                                ...
                                -- Pass 5: Find any checked checkbox (ultimate fallback to toggle off whatever is active)
                                if chosenBtn is missing value then
                                    repeat with cb in allCheckboxes
                                        try
                                            set cbValue to value of cb
                                ...
                                if chosenBtn is not missing value then
                                    ...
                                    error "Could not find any active screen mirroring connection to disconnect"
                                end if
                            end tell
                        end tell
                    end tell
        ...
        let error = scriptExecutor.executeScript(scriptSource)
        if let error = error {
            print("AppleScript disconnection execution failed: \(error)")
            if let url = URL(string: "x-apple.systempreferences:com.apple.Displays-Settings.extension") {
                NSWorkspace.shared.open(url)
            }
        }
